...
export class DealerLogoComponent implements OnInit {
  ...
  getAllLogoList() {
    ...
    this.customerServiceService.LogoList().subscribe((res) => {
      if (res?.status == true) {
  ...
  openDeleteDialog(id: any) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result == true) {
        this.spinnerService.show();
        this.customerServiceService.deleteLogo(id).subscribe((res) => {
          if (res?.status == true) {
            this.toastr.success(res?.message);
            this.getAllLogoList();
            this.spinnerService.hide();
...
export class LogoEditDialogComponent implements OnInit {
  ...
  updateLogo() {
    ...
    if (this.data?.logo_id) {
      this.spinnerService.show();
      this.customerServiceService.updateLogoDetails(postData).subscribe((res) => {
        if (res?.status == true) {
          this.toastr.success(res?.message);
          this.dialogRef.close();
          this.spinnerService.hide();
